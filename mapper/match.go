...
func FindBestMatch(currentUserEmbedding *pgvector.Vector, currentUserGender string) (*model.MatchProfile, float64, error) {
	...
	err := global.DB.Table("match_profiles").
		Select("*, (1 - (partner_embedding <=> ?)) AS similarity", currentUserEmbedding).
		Where("gender = ? and is_deleted = ?", targetGender, 0).
